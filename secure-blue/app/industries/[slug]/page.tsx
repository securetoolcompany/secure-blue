...
import { 
  ...
  ChevronRight, Target, CheckCircle2, Box, AlertTriangle, 
  Droplets, Wind, Zap, Gauge, Building2, Flag, Tractor, Landmark, Mountain, Flame
} from 'lucide-react';
import Link from 'next/link';
import ContactForm from '@/components/ContactForm'; // Utilizing your existing form!
    ...
    tracks: [
      {
        ...
        icon: Droplets,
        goals: ['Eliminate over-watering and root rot.', 'Drastically reduce municipal water bills.', 'Automate delivery based on live API weather data.'],
        deliverables: ['LoRaWAN Soil Sensor array.', 'Automated actuator integration.', 'Live topography moisture mapping dashboard.']
      },
      {
        ...
        icon: ShieldCheck,
        goals: ['Prevent structural water damage.', 'Lower commercial insurance premiums.', 'Maintain uninterrupted guest experiences.'],
        deliverables: ['Holistic intake flow sensors.', 'Algorithmic baseline anomaly detection.', 'Millisecond auto-shutoff valves.']
      }
    ],
    terminalLogs: [
    ...
    tracks: [
      {
        ...
        icon: Wind,
        goals: ['Insulate crops from government water rationing.', 'Create a secondary, reliable water supply.', 'Ensure pure, contaminant-free water for sensitive crops.'],
        deliverables: ['Industrial A2W generator banks.', 'Solar/Power matrix integration.', 'Live atmospheric yield tracking.']
      },
      {
        ...
        icon: Target,
        goals: ['Maximize crop yield per gallon.', 'Prevent fertilizer runoff from over-watering.', 'Monitor thousands of acres from one screen.'],
        deliverables: ['Ruggedized deep-soil sensor probes.', 'Long-range mesh network gateways.', 'Automated drip-line actuation.']
      }
    ],
    terminalLogs: [
    ...
    tracks: [
      {
        ...
        icon: ShieldCheck,
        goals: ['Prevent catastrophic multi-floor flooding.', 'Negotiate massive commercial insurance discounts.', 'Protect sensitive tenant server rooms/assets.'],
        deliverables: ['Main-line holistic flow sensors.', 'Industrial-grade automated ball valves.', 'Instant SMS/Email threat alerts.']
      },
      {
        ...
        icon: Activity,
        goals: ['Eliminate silent utility bleed.', 'Sub-meter tenant usage accurately.', 'Increase overall building Net Operating Income (NOI).'],
        deliverables: ['Live GPM consumption dashboard.', 'Historical flow analytics.', 'Automated ROI and savings reports.']
      }
    ],
    terminalLogs: [
    ...
    tracks: [
      {
        ...
        icon: Droplets,
        goals: ['Drastically reduce civic water consumption.', 'Automate public works maintenance schedules.', 'Publicly demonstrate green-tech initiatives.'],
        deliverables: ['Wide-area LoRaWAN municipal networks.', 'Smart actuator retrofitting for public spaces.', 'Centralized city-wide dashboard.']
      },
      {
        ...
        icon: Zap,
        goals: ['Create resilient disaster-relief water points.', 'Relieve strain on aging treatment plants.', 'Ensure critical facilities remain operational.'],
        deliverables: ['High-capacity A2W deployment.', 'Graywater recycling loops for civic buildings.', 'Independent power/water matrices.']
      }
    ],
    terminalLogs: [
    ...
    tracks: [
      {
        ...
        icon: ShieldCheck,
        goals: ['Prevent catastrophic drain of shared water supplies.', 'Create independent water reserves for mountain estates.', 'Stabilize and lower annual HOA utility dues.'],
        deliverables: ['Main-line LeakStop actuation for winter bursts.', 'Community-scale A2W atmospheric generators.', 'Live reservoir/tank volume telemetry.']
      },
      {
...
export default async function IndustryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!industry) {
    return <div className="min-h-screen bg-zinc-950 text-white flex items-center justify-center font-mono">SECTOR_NOT_FOUND</div>;
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-zinc-950 text-zinc-50 selection:bg-zinc-800">
      ...
      <section className="relative px-8 pt-24 pb-16 max-w-7xl mx-auto w-full border-b border-zinc-800/50">
        <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-full h-[400px] ${industry.borderGlow.replace('border-', 'bg-').replace('/50', '/5')} blur-[120px] rounded-full pointer-events-none`} />
      ...
      <section className="px-8 py-24 bg-zinc-950 border-b border-zinc-800/50 relative overflow-hidden">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] ${industry.borderGlow.replace('border-', 'bg-').replace('/50', '/5')} rounded-full blur-[100px] pointer-events-none`} />
        ...
        <div className="max-w-5xl mx-auto relative z-10 border border-zinc-800 bg-zinc-900/40 p-10 md:p-16 shadow-2xl">
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <div className="space-y-6 text-zinc-400 leading-relaxed font-light">
              <p>{industry.paradigmText1}</p>
            </div>
            <div className="space-y-6 text-zinc-400 leading-relaxed font-light">
              <p>{industry.paradigmText2}</p>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-16">
            ...
            <p className="text-zinc-400 text-lg leading-relaxed max-w-3xl">
              We provide turn-key, zero-overhead IoT pipelines specifically engineered for the {industry.name} sector. Take total control of your water management.
            </p>
          </div>
          ...
          <div className="space-y-12">
            {industry.tracks.map((track, idx) => {
              ...
              return (
                <div key={idx} className={`border border-zinc-800 bg-zinc-900/30 p-8 hover:${industry.borderGlow.replace('/50', '/30')} transition-all group relative overflow-hidden`}>
                  <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                    <TrackIcon className="w-48 h-48" />
                  </div>
                  <div className="relative z-10 grid lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-1">
                      <div className={`inline-flex items-center rounded ${industry.borderGlow.replace('border-', 'bg-').replace('/50', '/10')} px-2 py-1 text-xs font-mono ${industry.accentColor} mb-4 border ${industry.borderGlow.replace('/50', '/20')}`}>
                        {track.id}
                      </div>
                      ...
                      <div>
                        ...
                        <ul className="space-y-2 text-xs text-zinc-400">
                          {track.goals.map((goal, gIdx) => (
                            <li key={gIdx} className="flex items-start"><span className={`${industry.accentColor} mr-2`}>-</span> {goal}</li>
                          ))}
                        </ul>
                      </div>
                      <div>
                        ...
                        <ul className="space-y-2 text-xs text-zinc-400">
                          {track.deliverables.map((deliv, dIdx) => (
                            <li key={dIdx} className="flex items-start"><span className="text-emerald-400 mr-2">-</span> {deliv}</li>
                          ))}
                        </ul>
                      </div>
                    </div>
        ...
        <div className="max-w-4xl mx-auto rounded-md border border-zinc-800 bg-zinc-950 overflow-hidden shadow-2xl">
          ...
          <div className="p-8 font-mono text-sm space-y-4">
            {industry.terminalLogs.map((log, lIdx) => (
              <div key={lIdx} className={log.key === 'LOG' ? `mt-4 text-xs ${log.val.includes('WARNING') ? 'text-red-400' : 'text-zinc-400'}` : "flex justify-between text-zinc-500 border-b border-zinc-800 pb-2"}>
                {log.key === 'LOG' ? (
                  <p>{log.val}</p>
                ) : (
                  <>
                    <span>{log.key}:</span><span className="text-zinc-300">{log.val}</span>
                  </>
                )}
              </div>
            ))}
      ...
      <section id="initiate-contact" className="px-8 py-24 bg-zinc-950">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="border border-zinc-800 bg-zinc-900/50 p-8 shadow-2xl relative overflow-hidden min-h-[400px]">
            ...
            {/* Re-using your exact form component, passing the industry name instead of product name */}
